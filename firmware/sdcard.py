...
class SDCard:
    ...
    def init_card(self, baudrate):
        ...
        # NOTE: on this board the R1 byte glitches on the bus turnaround for CMD8
        # specifically, so decide from the R7 echo payload (0x01AA) rather than R1.
        self.cs(0)
        buf = self.cmdbuf
        buf[0] = 0x40 | 8
        buf[1] = 0x00
        ...
        self.cs(1)
        self.spi.write(b"\xff")
        if any(b == 0xAA for b in r7):  # voltage-check echo present -> v2/SDHC
            self.init_card_v2()
        elif any(b == (_R1_IDLE_STATE | _R1_ILLEGAL_COMMAND) for b in r7):
            self.init_card_v1()
        else:
    ...
    def readinto(self, buf):
        ...
        for i in range(_CMD_TIMEOUT):
            ...
            # drain any late/partial data block (CS still low) so a marginal
            # transfer doesn't leave the card wedged until a power cycle
            for _ in range(520):
                self.spi.write(b"\xff")
            self.cs(1)
            self.spi.write(b"\xff")
    ...
    def readblocks(self, block_num, buf):
        ...
        if nblocks == 1:
            # CMD17 + read, with retries to ride out marginal transfers
            last = None
            for _try in range(5):
                if self.cmd(17, block_num * self.cdv, release=False) != 0:
                    ...
                    try:
                        ...
                        last = e
                # settle the bus before retrying
                for _ in range(80):
                    self.spi.write(b"\xff")
                time.sleep_ms(2)
            raise last
